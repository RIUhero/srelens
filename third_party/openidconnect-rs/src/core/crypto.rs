...
pub fn verify_rsa_signature(
    key: &CoreJsonWebKey,
    parameters: &'static ring::signature::RsaParameters,
    msg: &[u8],
    signature: &[u8],
) -> Result<(), SignatureVerificationError> {
    let (n, e) = rsa_public_key(key).map_err(SignatureVerificationError::InvalidKey)?;
    // RFC 7518 requires unsigned integers to use the minimum number of octets,
    // but retain compatibility with providers that include leading zeroes.
    let n = trim_leading_zeroes(n.deref());
    let e = trim_leading_zeroes(e.deref());
