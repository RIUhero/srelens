...
impl CoreRsaPrivateSigningKey {
    ...
    pub fn from_pem(pem: &str, kid: Option<JsonWebKeyId>) -> Result<Self, String> {
        let (label, der) = pem_rfc7468::decode_vec(pem.as_bytes()).map_err(|err| err.to_string())?;
        if label != "RSA PRIVATE KEY" {
            return Err(format!("expected RSA PRIVATE KEY, found {label}"));
        }
...
impl PrivateSigningKey for CoreRsaPrivateSigningKey {
    ...
    fn sign(
        ...
        let encoding: &'static dyn ring::signature::RsaEncoding = match *signature_alg {
            CoreJwsSigningAlgorithm::RsaSsaPkcs1V15Sha256 => &ring::signature::RSA_PKCS1_SHA256,
            CoreJwsSigningAlgorithm::RsaSsaPkcs1V15Sha384 => &ring::signature::RSA_PKCS1_SHA384,
            CoreJwsSigningAlgorithm::RsaSsaPkcs1V15Sha512 => &ring::signature::RSA_PKCS1_SHA512,
            CoreJwsSigningAlgorithm::RsaSsaPssSha256 => &ring::signature::RSA_PSS_SHA256,
            CoreJwsSigningAlgorithm::RsaSsaPssSha384 => &ring::signature::RSA_PSS_SHA384,
            CoreJwsSigningAlgorithm::RsaSsaPssSha512 => &ring::signature::RSA_PSS_SHA512,
            ref other => {
    ...
    fn as_verification_key(&self) -> CoreJsonWebKey {
        ...
        CoreJsonWebKey {
            ...
            use_: Some(CoreJsonWebKeyUse::Signature),
            kid: self.kid.clone(),
            n: Some(Base64UrlEncodedBytes::new(public_key.n)),
            e: Some(Base64UrlEncodedBytes::new(public_key.e)),
            k: None,
            crv: None,
            x: None,
            y: None,
